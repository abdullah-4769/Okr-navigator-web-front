...
class SuggestionInitiativesViewModel extends GetxController {
  ...
  /// ✅ Save strategy and objective data to SharedPreferences for challenge mode
  Future<void> _saveStrategyAndObjectiveToPrefs() async {
    try {
      ...
      final String strategyTitle = strategyController.selectedStrategy.value?.title ?? '';
      final String objectiveTitle = objectiveController.selectedTitleKey ?? '';

      if (strategyTitle.isNotEmpty) {
        await SharedPrefs.saveSelectedStrategy({'title': strategyTitle});
        print('💾 Saved strategy: $strategyTitle');
      }

      if (objectiveTitle.isNotEmpty) {
        await SharedPrefs.saveSelectedObjective({'title': objectiveTitle});
        print('💾 Saved objective: $objectiveTitle');
      }
    } catch (e) {
  ...
  /// ✅ FIXED: Handle both List<KeyResult> and List<Map<String, dynamic>> types
  Future<void> submitInitiatives(dynamic selectedKeyResults) async {
    print('🎯 Submit Initiatives Called');
    print('   - _isModifyFromContextual: $_isModifyFromContextual');
    print('   - Selected Key Results Type: ${selectedKeyResults.runtimeType}');
    print('   - Selected Key Results: $selectedKeyResults');

    // ✅ Convert to List<KeyResult> if it's List<Map<String, dynamic>>
    List<KeyResult> keyResultsList = [];

    if (selectedKeyResults is List<KeyResult>) {
      keyResultsList = selectedKeyResults;
      print('✅ Already List<KeyResult>, using directly');
    } else if (selectedKeyResults is List<Map<String, dynamic>>) {
      print('🔄 Converting List<Map> to List<KeyResult>');
      try {
        keyResultsList = selectedKeyResults.map((map) => KeyResult.fromJson(map)).toList();
        print('✅ Successfully converted ${keyResultsList.length} key results');
      } catch (e) {
        print('❌ Error converting Map to KeyResult: $e');
        Get.snackbar(
          'error'.tr,
          'Error processing key results',
      ...
      print('❌ Unknown type: ${selectedKeyResults.runtimeType}');
      Get.snackbar(
        'error'.tr,
        'Invalid key results data',
        snackPosition: SnackPosition.BOTTOM,
        backgroundColor: Colors.red,
        colorText: Colors.white,
      );
    ...
    try {
      ...
      // 🔹 SAVE INITIATIVES BEFORE SUBMISSION
      await saveInitiativesToStorage();

      // ✅ Save strategy/objective to SharedPreferences for challenge mode
      await _saveStrategyAndObjectiveToPrefs();

      // ✅ Get strategy and objective data with multiple fallbacks
      ...
      // METHOD 1: Try to get from SharedPreferences first
      ...
      if (objectiveTitle.isEmpty) {
        try {
          final objectiveController = Get.find<KeyObjectiveController>();
          objectiveTitle = objectiveController.selectedTitleKey ?? '';
      ...
      if (strategyTitle.isEmpty) {
        strategyTitle = 'Strategic Partnerships';
        print('⚠️ Using default strategy: $strategyTitle');
      }

      if (objectiveTitle.isEmpty) {
        objectiveTitle = 'Establish Local Partnerships';
      ...
      print('Key Results Count: ${keyResultsList.length}');

      if (strategyTitle.isEmpty || objectiveTitle.isEmpty) {
        print('Validation failed: Strategy or Objective is empty');
      ...
      final response = await _repository.evaluateInitiatives(
        ...
        keyResults: keyResultsList, // ✅ Use the converted list
        language: language,
      ...
      if (response.status == Status.completed && response.data != null) {
        ...
        // ✅ FIXED: DIFFERENT NAVIGATION BASED ON SOURCE
        print('🚀 About to navigate to: ${_isModifyFromContextual ? AppRoutes.contextualChallenge : AppRoutes.aiAnalysisShowScreen}');

        if (_isModifyFromContextual) {
          // Navigate to contextual challenge screen with revised initiatives data
          print('🔄 Navigating to Contextual Challenge screen');
          Get.offAllNamed(
            AppRoutes.contextualChallenge,
            arguments: {
              'revisedInitiatives': response.data,
              'source': 'initiatives_revision',
              'firstInitiative': firstInitiativeTitle.text.trim(),
              'firstInitiativeDesc': firstInitiativeDesc.text.trim(),
              'secondInitiative': secondInitiativeTitle.text.trim(),
              'secondInitiativeDesc': secondInitiativeDesc.text.trim(),
            },
          );
        } else {
