...
class ChallengeViewModel extends GetxController {
  ...
  Future<void> fetchChallenge() async {
    ...
    try {
      ...
      final strategyTitle = strategyController.selectedStrategyTitle;
      final objectiveData = objectiveController.selectedObjective;

      // FIXED: Use currentLanguage getter instead of selectedLanguage.value
      final language = languageController.currentLanguage;

      final keyResultTitle = 'default_key_result';

      final objectiveTitle = objectiveData?['titleKey'] ?? '';
      ...
      final response = await _repository.getChallenge(
        ...
        language: language, // Now this is a String, not RxString
      );
