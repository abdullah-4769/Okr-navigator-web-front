...
class FeedbackEvaluationViewModel extends GetxController {
  ...
  Future<String> _getStrategyTitle() async {
    try {
      ...
      // FIXED: selectedStrategy is RxString, so just check if it's not empty
  ...
  Future<String> _getObjectiveTitle() async {
    try {
      // Try controller first - FIXED: KeyObjectiveController uses index-based selection
      final objectiveController = Get.find<KeyObjectiveController>();

      // FIXED: Use the selectedTitleKey getter or check selectedIndex
      if (objectiveController.selectedIndex.value != -1) {
