...
class RoleSelectionScreen extends StatelessWidget {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        CustomHeader(
                          title: trKey('select'),
                          highlightedText: trKey('your_role'),
                          onBackTap: () => Get.offAllNamed(AppRoutes.pricingScreen),
                        ),
                        ...
                        Padding(
                          padding: EdgeInsets.symmetric(horizontal: AppDimensions.d20.w),
                          child: isPortrait
                            ? _buildPortraitLayout(context, orientation)
                            : _buildLandscapeLayout(context, orientation),
                        ),
  ...
  Widget _buildPortraitLayout(BuildContext context, Orientation orientation) {
    return Column(
      children: [
        // Welcome Section
        _buildWelcomeSection(context, orientation),
        SizedBox(height: AppDimensions.d10.h),

        // Instruction
        _buildInstruction(context, orientation),
        SizedBox(height: AppDimensions.d14.h),

        // Roles Grid
        _buildRolesGrid(context, orientation, isFullWidth: true),
        SizedBox(height: AppDimensions.d20.h),

        // Continue Button & Tutorial
        _buildBottomSection(context, orientation),
      ],
    );
  }

  // Landscape layout (side by side)
  Widget _buildLandscapeLayout(BuildContext context, Orientation orientation) {
    return Row(
      ...
      children: [
        ...
        Expanded(
          flex: 4,
          child: Padding(
            padding: EdgeInsets.only(top: AppDimensions.d20.h),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                ...
                Padding(
                  padding: EdgeInsets.only(right: AppDimensions.d20.w),
                  child: CustomButton2(
                    text: trKey('select_continue'),
                    onPressed: controller.continueWithSelection,
                  ),
                ...
                SizedBox(height: AppDimensions.d12.h),
                _buildTutorialLink(context, orientation),
              ],
            ),
          ),
        ),

        SizedBox(width: AppDimensions.d30.w),

        // Right Side - Roles Grid
        Expanded(
          flex: 5,
          child: _buildRolesGrid(context, orientation, isFullWidth: false),
        ),
      ],
    );
  }

  Widget _buildWelcomeSection(BuildContext context, Orientation orientation) {
    return Column(
      ...
      children: [
        Text(
          ...
          style: Theme.of(context).textTheme.displayMedium?.copyWith(
            color: AppColors.primaryRed,
            fontWeight: FontWeight.bold,
            fontSize: orientation == Orientation.portrait ? 20.sp : 22.sp,
          ),
        ...
        Text(
          ...
          style: Theme.of(context).textTheme.titleSmall?.copyWith(
            height: 1.4,
            fontSize: orientation == Orientation.portrait ? 14.sp : 13.sp,
          ),
          textAlign: orientation == Orientation.portrait
            ? TextAlign.center
            : TextAlign.left,
  ...
  Widget _buildInstruction(BuildContext context, Orientation orientation) {
    return Text(
      trKey('choose_role_instruction'),
      style: Theme.of(context).textTheme.displaySmall?.copyWith(
        fontWeight: FontWeight.w600,
        fontSize: orientation == Orientation.portrait ? 15.sp : 14.sp,
      ),
      textAlign: orientation == Orientation.portrait
        ? TextAlign.center
        : TextAlign.left,
  ...
  Widget _buildRolesGrid(BuildContext context, Orientation orientation, {required bool isFullWidth}) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (isFullWidth) {
          childAspectRatio = (screenWidth / crossAxisCount) / (screenHeight * (orientation == Orientation.portrait ? 0.38 : 0.55));
        } else {
          // For landscape side-by-side layout - adjusted for content
        ...
        return GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            childAspectRatio: childAspectRatio.clamp(0.65, 1.0),
          ),
          itemBuilder: (context, index) {
            final role = controller.roles[index];
            return _RoleCard(
  ...
  Widget _buildBottomSection(BuildContext context, Orientation orientation) {
    return Column(
      children: [
        CustomButton2(
          text: trKey('select_continue'),
  ...
  Widget _buildTutorialLink(BuildContext context, Orientation orientation) {
    return Row(
...
// ✅ Role Card with expandable content
class _RoleCard extends StatelessWidget {
  final int index;
  final Map<String, dynamic> role;
  final RoleSelectionController controller;
      ...
      return GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          padding: EdgeInsets.symmetric(
            horizontal: isLandscape ? AppDimensions.d8.w : AppDimensions.d12.w,
            vertical: isLandscape ? AppDimensions.d8.h : AppDimensions.d14.h,
          ),
          decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(AppDimensions.d16.r),
          ...
          child: Stack(
            ...
            children: [
              Column(
                ...
                children: [
                  // Avatar Icon with background circle
                  Stack(
                    alignment: Alignment.center,
                    children: [
                      // Background circle for icon
                      Container(
                        width: isLandscape ? 45.w : 60.w,
                        height: isLandscape ? 45.w : 60.w,
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          color: selected
                      ...
                      CustomSvg(
                        assetPath: role['asset'],
                        semanticsLabel: trKey(role['title']),
                        width: isLandscape ? 32.w : 45.w,
                        height: isLandscape ? 32.w : 45.w,
                      ),
                    ],
                  ),

                  SizedBox(height: isLandscape ? AppDimensions.d6.h : AppDimensions.d10.h),
                  ...
                  SizedBox(height: isLandscape ? AppDimensions.d4.h : AppDimensions.d4.h),

                  // Role Subtitle - Show full when selected, truncated when not
                  AnimatedCrossFade(
                    duration: const Duration(milliseconds: 260),
                  ...
                  // Extra Info - Show when selected
                  AnimatedSize(
                    duration: const Duration(milliseconds: 260),
                    curve: Curves.easeInOut,
                    child: selected && role['extra'] != null && role['extra'].toString().isNotEmpty
                      ? Padding(
                          padding: EdgeInsets.only(top: isLandscape ? AppDimensions.d4.h : AppDimensions.d4.h),
                          child: Text(
                            trKey(role['extra']),
                            style: Theme.of(context).textTheme.bodySmall?.copyWith(
                              color: AppColors.grey,
              ...
              Positioned(
                ...
                right: -4,
                child: Container(
                  padding: EdgeInsets.all(isLandscape ? AppDimensions.d4.w : AppDimensions.d6.w),
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: role['iconBg'],
                    boxShadow: [
