...
bool _isTablet(double sw)  => sw >= 600 && sw < 900;
bool _isDesktop(double sw) => sw >= 900;
...
class CustomBubbleButton extends StatefulWidget {
  ...
  @override
  State<CustomBubbleButton> createState() => _CustomBubbleButtonState();
}

class _CustomBubbleButtonState extends State<CustomBubbleButton>
  ...
  Widget build(BuildContext context) {
    ...
    final double fontSize = _isDesktop(sw)
        ? 14.0
        : _isTablet(sw)
        ? 13.0
        : 12.sp;

    final double btnHeight = _isDesktop(sw)
        ? widget.height.clamp(30.0, 52.0)
        : _isTablet(sw)
        ? widget.height.clamp(28.0, 48.0)
        : widget.height.clamp(26.0, 44.0);

    final double btnWidth = widget.isFullWidth
        ? (_isDesktop(sw)
        ? sw * 0.6
        : _isTablet(sw)
        ? sw * 0.75
        : sw * 0.88)
        : (_isDesktop(sw)
        ? widget.width.clamp(80.0, 260.0)
        : _isTablet(sw)
        ? widget.width.clamp(70.0, sw * 0.55)
        : widget.width.clamp(60.0, sw * 0.6));

    final double radius = _isDesktop(sw) ? 26.0 : _isTablet(sw) ? 24.0 : 22.r;

    final double borderWidth = _isDesktop(sw) ? 1.75 : 1.5;

    final EdgeInsets padding = _isDesktop(sw)
    ...
    final List<BoxShadow> shadow = [
      BoxShadow(
        color: Colors.black.withOpacity(_isDesktop(sw) ? 0.12 : 0.08),
        blurRadius: _isDesktop(sw) ? 6 : 3,
        offset: Offset(0, _isDesktop(sw) ? 3 : 1),
      ),
    ];
          ...
          child: FittedBox(
            ...
            child: Text(
              ...
              style: TextStyle(
                ...
                fontWeight: _isDesktop(sw) ? FontWeight.w700 : FontWeight.w600,
                fontSize: fontSize,
                letterSpacing: _isDesktop(sw) ? 0.2 : 0.0,
                height: 1.2,
              ),
            ),
          ),
...
// ─────────────────────────────────────────────────────────────────────────────
// Simplified wrapper for quick use
// ─────────────────────────────────────────────────────────────────────────────
class ResponsiveBubbleButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomBubbleButton(
      text: text,
      width: _isDesktop(sw) ? 200 : _isTablet(sw) ? 180 : 160,
      height: _isDesktop(sw) ? 46 : _isTablet(sw) ? 42 : 38,
      onTap: onTap,
